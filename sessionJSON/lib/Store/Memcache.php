...
/**
 * copied from SimpleSAML_Memcache - modified to enable raw set/get
 *
 *
 ...
 * @version $Id$
 */
class sspmod_sessionJSON_Store_Memcache {
	...
	/**
	 * Find data stored with a given key - JSON object
	 *
	 * @param $key  The key of the data.
	 * @return The data stored with the given key, or NULL if no data matching the key was found.
	 */
	public static function jsonget($key) {
		...
		foreach(self::getMemcacheServers() as $server) {
			...
			/* Deserialize the object as an array only. */
			$info = json_decode($serializedInfo, true);
	...
	/**
	 * Save a key-value pair to the memcache servers - JSON object.
	 *
	 * @param $key    The key of the data.
	 * @param $value  The value of the data.
	 * @param int|NULL $expire  The expiration timestamp of the data.
	 */
	public static function jsonset($key, $value, $expire = NULL) {
		$savedInfo = array(
			'timestamp' => microtime(TRUE),
			'data' => $value
			);
		...
		$savedInfoSerialized = json_encode($savedInfo);
